# Semgrep — marketplace-dev-public-authorization
...
`custom.yml` carries the two secret-in-logs rules common to every service —
`marketplace-no-log-introspection-code` (the `x-introspectioncode` /
`INTROSPECTION_CODE` service-to-service bypass secret) and
`marketplace-no-log-auth-token` (opaque access/refresh tokens). It deliberately
does **not** carry `marketplace-no-log-reset-secret`: that rule guards the
reset-password flow (`resetHash` / `resetDateReq`), which lives only in
`marketplace-dev-public-resource` (`src/lib/access/resetPwdFlow.mts`). This
service has no such flow.
...
- **Scope is the explicit file list, not `.semgrepignore`.** Because files are
  ...
  A merge-leftover `.orig` file, should one ever appear under `src/` (see
  CLAUDE.md), is correctly out of scope — the `find` list has no `.orig`
  entry.

...
// koa-utils is in the list too, unlike the other services: the login mutations wrap its
// LoginAppType in a `new GraphQLNonNull(...)`, so its graphql copy must be the same one.
// marketplace-common is in the list for a second, subtler reason: its checkUserAuthorizationDisDel
// throws a GraphQLError, and tryCatchRethrow (koa-utils, inlined) narrows it with `instanceof`.
// Left externalized, marketplace-common gets the native graphql copy, the instanceof is false, and a
// disabled/deleted login answers 500 "Internal Server Error" instead of 401 "Unauthorized" — a
// failure that exists only under vitest, since plain node resolves both to one copy.

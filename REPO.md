...
`yarn test:mutation` runs only when the `pre-push` hook calls it — not to check a change, not before a
commit, not on one file, not to confirm a survivor is fixed. Do not invoke `stryker` directly either.
...
names the tests that should have failed, and it costs nobody the machine. See [`CLAUDE.md`](./CLAUDE.md)
for the rule itself.

...
export function logListening(env: NodeJS.ProcessEnv = process.env): void {
	// `*` is deliberate and accurate: the server binds every interface (see start()), so there
	// is no single address to name — printing one would just be the same stale HOSTNAME lie in
	// a new place.

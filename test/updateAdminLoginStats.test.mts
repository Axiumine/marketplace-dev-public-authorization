...
vi.mock('@thedoctorweb_agency/marketplace-common/models/MongoDB/Admin', () => ({ Admin: { updateOne } }))

const { updateAdminLoginStats } = await import('../src/lib/db/login/updateAdminLoginStats.mts')
...
describe('updateAdminLoginStats', () => {
	...
	it('stamps firstLogin too, and sets rememberMe, on the very first login', async () => {
		await updateAdminLoginStats(_id, null, true, session)
	...
	it('only refreshes lastLogin and unsets rememberMe afterwards', async () => {
		await updateAdminLoginStats(_id, new Date('2026-01-01T00:00:00.000Z'), false, session)

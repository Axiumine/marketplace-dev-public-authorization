...
// tsconfig path aliases actually used by the sources. tsconfig.json also declares `@ptypes/*`
// pointing at `src/graphQLApi/schema/types/*`, a directory this service does not have (the types
// live under src/graphQLPublic/); no source imports it, so it is left out rather than repointed.
const aliases: Array<[string, string]> = [['@lib/', 'src/lib/']]
